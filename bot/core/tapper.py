...
async def run_tapper1(tg_clients: list[Client], proxies):
    ...
    while True:
        ...
        await asyncio.sleep(sleep_)        self.first_name = ''
        self.last_name = ''
        self.user_id = ''
        self.auth_token = ""
        self.last_claim = None
            ...
            if res.status_code == 200:
                ...
                for card in cards:
                    if card['boughtAt'] is not None:
                        if int(time_module.time())*1000 < card['boughtAt'] + card['rechargingDuration']:
                            continue
                    if card['condition'] is None and int(self.balance) >= int(card['nextLevel']['cost']) or card['condition']['passed'] and int(self.balance) >= int(card['nextLevel']['cost']):
                        available_cards.append(card)
